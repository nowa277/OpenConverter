/**
 * KGM / KGMA / VPR (KuGou Music v2 cache) decoder — pure JS, no native deps.
 *
 * Re-implemented from scratch based on huangbao/MyKgmWasm (MIT, 2025).
 * The MIT reference is ~150 LoC of pure cipher + 4 small lookup tables
 * (833 bytes total). No embedded xz blob, no 70MB key table.
 *
 * File layout:
 *   0x00..0x10  16-byte magic: KgmHeader (KGM/KGMA) or VprHeader (VPR)
 *   0x10..0x14  4-byte uint32 LE header length
 *   0x14..0x1C  4 bytes (reserved)
 *   0x1C..0x2C  16-byte per-file AES key (key[16] is implicitly 0)
 *   0x2C..headerLen  Reserved
 *   headerLen..end   Encrypted audio
 *
 * Decryption per byte at position i (offset = global position in audio):
 *   med8 = key[(i + offset) % 17] ^ blockData[i]
 *   med8 ^= (med8 & 0x0F) << 4
 *   msk8 = getMask(i + offset)
 *   msk8 ^= (msk8 & 0x0F) << 4
 *   blockData[i] = med8 ^ msk8
 *   if isVpr: blockData[i] ^= VprMaskDiff[(i + offset) % 17]
 *
 * getMask(pos):
 *   offset = pos >> 4
 *   value = 0
 *   while offset >= 0x11:
 *     value ^= table1[offset % 272]
 *     offset >>= 4
 *     value ^= table2[offset % 272]
 *     offset >>= 4
 *   return MaskV2PreDef[pos % 272] ^ value
 */
const fs = require('node:fs');
const path = require('node:path');

// 16-byte magic headers
const KGM_HEADER = Buffer.from([0x7C, 0xD5, 0x32, 0xEB, 0x86, 0x02, 0x7F, 0x4B, 0xA8, 0xAF, 0xA6, 0x8E, 0x0F, 0xFF, 0x99, 0x14]);
const VPR_HEADER = Buffer.from([0x05, 0x28, 0xBC, 0x96, 0xE9, 0xE4, 0x5A, 0x43, 0x91, 0xAA, 0xBD, 0xD0, 0x7A, 0xF5, 0x36, 0x31]);

// 17-byte VPR post-mask (key[16] is implicitly 0, so this maps both 0x00..0x10)
const VPR_MASK_DIFF = Buffer.from([0x25, 0xDF, 0xE8, 0xA6, 0x75, 0x1E, 0x75, 0x0E, 0x2F, 0x80, 0xF3, 0x2D, 0xB8, 0xB6, 0xE3, 0x11, 0x00]);

// 272-byte lookup tables (16*17 = 272)
const MASK_V2_PRE_DEF = Buffer.from([
  ,
]);

const TABLE1 = Buffer.from([
  ,
]);

const TABLE2 = Buffer.from([
  ,
]);

// Audio format headers (used by inferFormat)
const FLAC_HDR = Buffer.from([0x66, 0x4c, 0x61, 0x43]);
const MP3_HDR = Buffer.from([0x49, 0x44, 0x33]);
const OGG_HDR = Buffer.from([0x4f, 0x67, 0x67, 0x53]);
const M4A_HDR = Buffer.from([0x66, 0x74, 0x79, 0x70]);
const WMA_HDR = Buffer.from([0x30, 0x26, 0xb2, 0x75, 0x8e, 0x66, 0xcf, 0x11, 0xa6, 0xd9, 0x00, 0xaa, 0x00, 0x62, 0xce, 0x6c]);
const WAV_HDR = Buffer.from([0x52, 0x49, 0x46, 0x46]);
const AAC_HDR = Buffer.from([0xff, 0xf1]);
const DFF_HDR = Buffer.from([0x46, 0x52, 0x4d, 0x38]);

function inferFormat(audio) {
  if (audio.length < 4) return 'mp3';
  if (audio[0] === MP3_HDR[0] && audio[1] === MP3_HDR[1] && audio[2] === MP3_HDR[2]) return 'mp3';
  if (audio[0] === FLAC_HDR[0] && audio[1] === FLAC_HDR[1] && audio[2] === FLAC_HDR[2] && audio[3] === FLAC_HDR[3]) return 'flac';
  if (audio[0] === OGG_HDR[0] && audio[1] === OGG_HDR[1] && audio[2] === OGG_HDR[2] && audio[3] === OGG_HDR[3]) return 'ogg';
  if (audio[0] === WAV_HDR[0] && audio[1] === WAV_HDR[1] && audio[2] === WAV_HDR[2] && audio[3] === WAV_HDR[3]) return 'wav';
  if (audio[0] === 0xff && (audio[1] & 0xe0) === 0xe0) return 'mp3';
  if (audio[0] === WMA_HDR[0] && audio.length >= WMA_HDR.length) {
    let match = true;
    for (let i = 0; i < WMA_HDR.length; i++) if (audio[i] !== WMA_HDR[i]) { match = false; break; }
    if (match) return 'wma';
  }
  if (audio[0] === AAC_HDR[0] && audio[1] === AAC_HDR[1]) return 'aac';
  if (audio[0] === DFF_HDR[0] && audio[1] === DFF_HDR[1] && audio[2] === DFF_HDR[2] && audio[3] === DFF_HDR[3]) return 'dff';
  if (audio.length >= 8 && audio[4] === M4A_HDR[0] && audio[5] === M4A_HDR[1] && audio[6] === M4A_HDR[2] && audio[7] === M4A_HDR[3]) return 'm4a';
  return 'mp3';
}

function getMask(pos) {
  let offset = pos >> 4;
  let value = 0;
  while (offset >= 0x11) {
    value ^= TABLE1[offset % 272];
    offset >>= 4;
    value ^= TABLE2[offset % 272];
    offset >>= 4;
  }
  return MASK_V2_PRE_DEF[pos % 272] ^ value;
}

/**
 * Decrypt KGM/KGMA/VPR in-place. Returns the decrypted audio bytes.
 */
function decryptBuffer(kgmBuf) {
  if (kgmBuf.length < 0x2C) {
    throw new Error(`KGM/VPR: file too small (${kgmBuf.length} < 0x2C)`);
  }
  const header = kgmBuf.slice(0, 16);
  let isVpr;
  if (header.equals(KGM_HEADER)) {
    isVpr = false;
  } else if (header.equals(VPR_HEADER)) {
    isVpr = true;
  } else {
    throw new Error('KGM/VPR: bad magic, expected KGM or VPR header');
  }

  const headerLen = kgmBuf.readUInt32LE(0x10);
  if (headerLen < 0x2C || headerLen > kgmBuf.length) {
    throw new Error(`KGM/VPR: invalid header length ${headerLen}`);
  }

  // 17-byte key: 16 bytes at offset 0x1C, then key[16] = 0
  const key = Buffer.alloc(17);
  kgmBuf.copy(key, 0, 0x1C, 0x2C);
  key[16] = 0;

  const audio = Buffer.from(kgmBuf.slice(headerLen));
  for (let i = 0; i < audio.length; i++) {
    let med8 = key[(i % 17)] ^ audio[i];
    med8 ^= (med8 & 0x0f) << 4;
    let msk8 = getMask(i);
    msk8 ^= (msk8 & 0x0f) << 4;
    audio[i] = med8 ^ msk8;
    if (isVpr) {
      audio[i] ^= VPR_MASK_DIFF[i % 17];
    }
  }
  return audio;
}

function decodeFile(inputPath, outputDir) {
  const input = fs.readFileSync(inputPath);
  const audio = decryptBuffer(input);
  const format = inferFormat(audio);

  const base = inputPath.replace(/\.(kgm|kgma|vpr)$/i, '');
  const name = base.split(/[\\/]/).pop();
  const outName = `${name}.${format}`;
  const outPath = outputDir ? path.join(outputDir, outName) : `${base}.${format}`;
  fs.mkdirSync(outputDir || '.', { recursive: true });
  fs.writeFileSync(outPath, audio);
  return { outputPath: outPath, format };
}

module.exports = {
  decryptBuffer,
  decodeFile,
  inferFormat,
  KGM_HEADER,
  VPR_HEADER,
};
