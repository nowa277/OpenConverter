...
object FfmpegArgs {
    ...
    fun build(
        ...
        val hasMetadataFile = !metadataFile.isNullOrBlank()
        val args = mutableListOf("-y", "-i", input)
        if (hasExternalCover) args += listOf("-i", coverPath!!)
        if (hasMetadataFile) args += listOf("-i", metadataFile!!)
        ...
        val metadataIndex = when {
            hasMetadataFile && hasExternalCover -> "2"
            hasMetadataFile -> "1"
            else -> "0"
        }
        args += listOf("-map_metadata", metadataIndex)
        if (!hasMetadataFile) {
            for ((k, v) in metadata) {
